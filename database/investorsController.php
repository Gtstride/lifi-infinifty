...
$investorTypes = $_POST['investor_type'] ?? [];
$otherInvestor = trim($_POST['other_investor_type'] ?? '');
$amount = trim($_POST['amount'] ?? '');
$amount = $amount === '' ? null : $amount;
$stage = $_POST['business_stage'] ?? '';
...
$investorTypeStr = implode(', ', $investorTypes);
if (in_array('Other', $investorTypes) && $otherInvestor !== '') {
    $investorTypeStr .= ' (Other: ' . $otherInvestor . ')';
}

// Save to database
$stmt = $pdo->prepare("INSERT INTO investors 
(full_name, company_name, email, phone, country, linkedin, investor_type, other_investor_type, amount, stage, industry, referral, comments) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

$stmt->execute([$name, $company, $email, $phone, $country, $linkedin, $investorTypeStr, $otherInvestor, $amount, $stage, $industry, $referral, $comments]);
...
try {
    ...
    // Email content
    $mail->isHTML(true);
    $mail->Subject = 'New Investor Submission';
    ...
    <div class='container'>
        ...
        <div class='info'>
            ...
            <p><strong>Investor Type:</strong> {$investorTypeStr}</p>
            <p><strong>Amount Interested:</strong> £{$amount}</p>
    ...
    if (!empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        <div class='container'>
            ...
            <div class='content'>
                <p>Dear {$name},</p>
                <p>Thank you for expressing your interest in investing with <strong>Lifi Infinity Ltd</strong>.</p>
